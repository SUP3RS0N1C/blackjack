...
int do_or_do_not()
{
	...
	get_theInteger = getIntegerInput();
...
{	int Choose_Action;
	...
	do{
		...
		{	//다시카드한장을 뽑도록한다  
			...
			pulledCard = pullCard();
			cardhold[0][cardCount[0]-1] = pulledCard;
	...
	}while(cardSum[0]<22&&continueNum==1&&Choose_Action==0);
...
{	int pulledCard;
	...
	do{
		...
		{	//다시카드한장을 뽑도록한다
			...
			cardCount[player]++;
			pulledCard = pullCard();
			cardhold[player][cardCount[player]-1] = pulledCard;
